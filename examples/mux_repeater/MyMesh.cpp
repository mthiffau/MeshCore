    ...
    if (!(flags == TXT_TYPE_PLAIN || flags == TXT_TYPE_CLI_DATA)) {
      ...
      if (is_retry) {
        *reply = 0;
        ...
        text_len = handleCommand(sender_timestamp, command, reply, (166 - 5));
      }
      if (text_len > 0) {
...
MyMesh::MyMesh(mesh::MainBoard &board, mesh::Radio &radio, mesh::MillisecondClock &ms, mesh::RNG &rng,
    ...
    : mesh::Mesh(radio, ms, rng, rtc, *new StaticPoolPacketManager(32), tables),
      ...
      , serial_bridge(&_prefs, WITH_RS232_BRIDGE, _mgr, &rtc)
#endif
...
uint16_t MyMesh::handleCommand(uint32_t sender_timestamp, char *command, char *reply,
  ...
  if (region_load_active) {
    if (StrHelper::isBlank(command)) {  // empty/blank line, signal to terminate 'load' operation
      ...
      reply_len = sprintf(reply, "%sOK - loaded %d regions", prefix, region_map.getCount());
    } else {
      char *np = command;
  ...
  if (memcmp(command, "setperm ", 8) == 0) {   // format:  setperm {pubkey-hex} {permissions-int8}
    ...
    if (sp == NULL) {
      reply_len = snprintf(reply, max_reply_len, "%s%sErr - bad params", prefix, cmd_prefix);
    } else {
      *sp++ = 0;   // replace space with null terminator
      ...
      if (mesh::Utils::fromHex(pubkey, hex_len / 2, hex)) {
        ...
        if (acl.applyPermissions(self_id, pubkey, hex_len / 2, perms)) {
          ...
          reply_len = snprintf(reply, max_reply_len, "%s%sOK", prefix, cmd_prefix);
        } else {
          reply_len = snprintf(reply, max_reply_len, "%s%sErr - invalid params", prefix,
                               cmd_prefix);
        }
      } else {
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - bad pubkey", prefix, cmd_prefix);
      }
    }
  } else if (sender_timestamp == 0 && strcmp(command, "get acl") == 0) {
    uint16_t written = 0;
    written = sprintf(reply, "ACL:\n");
    reply_len += written;
    reply += written;
    ...
    for (int i = 0; i < acl.getNumClients(); i++) {
      ...
      // 2 hex chars for perms byte, 1 space, 2 hex chars per pub key byte, 1 newline
      constexpr size_t ENTRY_LEN = 2 + 1 + (2 * PUB_KEY_SIZE) + 1;
      if (reply_len + ENTRY_LEN > max_reply_len) {
        MESH_DEBUG_PRINTLN("Buffer too short to write full ACL.");
        return reply_len;
      }

      written = sprintf(reply, "%02X ", c->permissions);
      reply_len += written;
      reply += written;

      mesh::Utils::toHex(reply, c->id.pub_key, PUB_KEY_SIZE);
      written = (2 * PUB_KEY_SIZE);
      reply_len += written;
      reply += written;

      written = sprintf(reply, "\n");
      reply_len += written;
      reply += written;
    }
    ...
    if (n == 1) {
      reply_len = sprintf(reply, "%s%s", prefix, cmd_prefix);
      reply += reply_len;
      reply_len += region_map.exportTo(reply, 160);
      ...
      reply_len = snprintf(reply, max_reply_len, success ? "%s%sOK" : "%s%sErr - save failed",
                           prefix, cmd_prefix);
      ...
      if (region) {
        ...
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - unknown region", prefix, cmd_prefix);
      }
      ...
      if (region) {
        ...
        if (parent && parent->id != 0) {
          reply_len = snprintf(reply, max_reply_len, "%s%s %s (%s) %s", prefix, cmd_prefix,
                               region->name, parent->name,
                               (region->flags & REGION_DENY_FLOOD) ? "" : "F");
        } else {
          reply_len = snprintf(reply, max_reply_len, "%s%s %s %s", prefix, cmd_prefix,
                               region->name, (region->flags & REGION_DENY_FLOOD) ? "" : "F");
        }
      } else {
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - unknown region", prefix, cmd_prefix);
      }
      ...
      if (home) {
        ...
        reply_len = snprintf(reply, max_reply_len, "%s%s home is now %s", prefix, cmd_prefix,
                             home->name);
      } else {
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - unknown region", prefix, cmd_prefix);
      }
      ...
      reply_len = snprintf(reply, max_reply_len, "%s%s home is %s", prefix, cmd_prefix,
                           home ? home->name : "*");
    } else if (n >= 3 && strcmp(parts[1], "put") == 0) {
      auto parent = n >= 4 ? region_map.findByNamePrefix(parts[3]) : &region_map.getWildcard();
      if (parent == NULL) {
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - unknown parent", prefix, cmd_prefix);
      } else {
        auto region = region_map.putRegion(parts[2], parent->id);
        if (region == NULL) {
          reply_len = snprintf(reply, max_reply_len, "%s%sErr - unable to put", prefix, cmd_prefix);
        } else {
          reply_len = snprintf(reply, max_reply_len, "%s%sOK", prefix, cmd_prefix);
        }
      }
      ...
      if (region) {
        if (region_map.removeRegion(*region)) {
          ...
          reply_len = snprintf(reply, max_reply_len, "%s%sErr - not empty", prefix, cmd_prefix);
        }
      } else {
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - not found", prefix, cmd_prefix);
      }
      ...
      if (strcmp(parts[2], "allowed") == 0) {
        ...
        reply_len = snprintf(reply, max_reply_len, "%s%sErr - use 'allowed' or 'denied'", prefix, 
                             cmd_prefix);
        return reply_len;
      }
      
      uint16_t written = 0;
      written = sprintf(reply, "%s%s", prefix, cmd_prefix);
      reply_len += written;
      reply += written;

      written = region_map.exportNamesTo(reply, 160, mask, invert);
      if (written == 0) {
        written = sprintf(reply, "-none-");
      }
      reply_len += written;
    } else {
      reply_len = snprintf(reply, max_reply_len, "%s%sErr - ??", prefix, cmd_prefix);
    }
  } else if (memcmp(command, "discover.neighbors", 18) == 0) {
    const char* sub = command + 18;
    while (*sub == ' ') sub++;
    if (*sub != 0) {
      reply_len = snprintf(reply, max_reply_len, "%s%SErr - discover.neighbors has no options",
                           prefix, cmd_prefix);
    } else {
      sendNodeDiscoverReq();
      reply_len = snprintf(reply, max_reply_len, "%s%sOK - Discover sent", prefix, cmd_prefix);
    }
  } else{
    reply_len = sprintf(reply, "%s%s", prefix, cmd_prefix);
    reply += reply_len;
...
void MyMesh::loop() {
  ...
  // Read serial port and dispatch to serial bridge or CLI handler as needed.
  while (_serial.available()) {
    uint8_t b = _serial.read();

    if (_rx_buffer_pos < 2) {
      // Waiting for the magic word for either a bridge packet or a CLI packet
      if ((_rx_buffer_pos == 0 && b == ((BridgeBase::BRIDGE_PACKET_MAGIC >> 8) & 0xFF)) ||
          (_rx_buffer_pos == 1 && b == (BridgeBase::BRIDGE_PACKET_MAGIC & 0xFF))) {
        ...
        if (b == ((BridgeBase::BRIDGE_PACKET_MAGIC >> 8) & 0xFF)) {
          _cur_pkt_type = PACKET_TYPE_BRIDGE;
          _rx_buffer[_rx_buffer_pos++] = b;
    ...
    if (_cur_pkt_type == PACKET_TYPE_BRIDGE && len > (MAX_TRANS_UNIT + 1)) {
      ...
      continue;
               ...
               len > (MAX_TEXT_PACKET_SIZE - SERIAL_OVERHEAD)) {
      MESH_DEBUG_PRINTLN("RX invalid length %d, resetting\n", len);
      _cur_pkt_type = PACKET_TYPE_UNKNOWN;
      _rx_buffer_pos = 0; // Invalid length, reset
      continue;
    }
    ...
    if (validateChecksum(_rx_buffer + 4, len, received_checksum)) {
      MESH_DEBUG_PRINTLN("RX, len=%d crc=0x%04x\n", len, received_checksum);
      if (_cur_pkt_type == PACKET_TYPE_BRIDGE) {
        ...
        if (pkt) {
...
#ifdef WITH_RS232_BRIDGE
            serial_bridge.onPacketReceived(pkt);
#endif
#ifdef WITH_ESPNOW_BRIDGE
            espnow_bridge.onPacketReceived(pkt);
#endif
          } else {
          ...
          MESH_DEBUG_PRINTLN("RX failed to allocate packet\n");
        }
#endif
      } else if (_cur_pkt_type == PACKET_TYPE_CLI) {
        // handleCommand expects a null terminated string. Now that we've verified the checksum,
        // it is safe to overwrite the first byte past the payload with 0.
        _rx_buffer[4 + len] = 0;
        // Reset the tx buffer to be an empty string.
        _tx_buffer[0] = 0;
        uint16_t reply_len = handleCommand(0, ((char*)_rx_buffer) + 4, (char*)_tx_buffer,
                                           MAX_TEXT_PACKET_SIZE);
        if (reply_len > 0) {
          sendTextPacket((char*)_tx_buffer, reply_len, PACKET_TYPE_CLI);
        }
      }
    } else {
      MESH_DEBUG_PRINTLN("RX checksum mismatch, rcv=0x%04x\n", received_checksum);
    }
...
#if defined(WITH_RS232_BRIDGE)
  if (serial_bridge.isRunning()) return true;  // bridge needs WiFi radio, can't sleep
#endif
#if defined(WITH_ESPNOW_BRIDGE)
  if (espnow_bridge.isRunning()) return true;  // bridge needs WiFi radio, can't sleep
#endif
  return _mgr->getOutboundCount(0xFFFFFFFF) > 0;
}

uint16_t MyMesh::fletcher16(const uint8_t *data, size_t len) {
...
bool MyMesh::validateChecksum(const uint8_t *data, size_t len, uint16_t received_checksum) {
  uint16_t calculated_checksum = fletcher16(data, len);
  return received_checksum == calculated_checksum;
}

void MyMesh::sendTextPacket(const char* text, uint16_t text_len, PacketType type) {
  // First validate the text pointer
  if (!text || text_len == 0) {
    MESH_DEBUG_PRINTLN("TX invalid text pointer\n");
    return;
  }

  // Build packet header
  uint8_t header[4];
  if (type == PACKET_TYPE_CLI) {
    header[0] = (CLI_PACKET_MAGIC >> 8) & 0xFF; // Magic high byte
    header[1] = CLI_PACKET_MAGIC & 0xFF;        // Magic low byte
  } else if (type == PACKET_TYPE_DEBUG) {
    header[0] = (DEBUG_PACKET_MAGIC >> 8) & 0xFF; // Magic high byte
    header[1] = DEBUG_PACKET_MAGIC & 0xFF;        // Magic low byte
  }
  header[2] = (text_len >> 8) & 0xFF;                 // Length high byte
  header[3] = text_len & 0xFF;                        // Length low byte

  // Calculate checksum over the payload
  uint16_t checksum = fletcher16((const uint8_t*)text, text_len);
  uint8_t checksum_bytes[2];
  checksum_bytes[0] = (checksum >> 8) & 0xFF; // Checksum high byte
  checksum_bytes[1] = checksum & 0xFF;        // Checksum low byte

  // Send complete packet
  _serial.write(header, 4);
  _serial.write(text, text_len);
  _serial.write(checksum_bytes, 2);

  MESH_DEBUG_PRINTLN("TX, len=%d crc=0x%04x\n", text_len, checksum);
}
