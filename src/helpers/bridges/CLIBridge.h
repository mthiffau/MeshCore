...
/**
 * @brief Packet type - CLI or bridge
 */
enum PacketType {
  PACKET_TYPE_UNKNOWN,
  PACKET_TYPE_CLI,
...
public:
  virtual void handleCommand(uint32_t sender_timestamp, char *command, char *reply) = 0;
};

/**
 * @brief Common magic number used by CLI bridge implementations for CLI packet identification
 *
 ...
 *
 * Packet Structure:
 * [2 bytes] Magic Header (0xC03E) - Used to identify start of CLIBridge packets
 ...
 */
class CLIBridge : public BridgeBase {
public:
  /**
   * @brief Constructs an CLIBridge instance
   *
   ...
   */
  void sendPacket(mesh::Packet *packet, PacketType type);
...
private:
  ...
  /** The type of the packet currently being read */
  PacketType _cur_pkt_type = PACKET_TYPE_UNKNOWN;
};
